...
async def join_voice(chann):
    global vClient
    global voice
    voice = await client.join_voice_channel(chann)
    print('Joined voice channel with session id '+voice.session_id)
    vClient = voice
...
async def on_message(message):
    ...
    if message.author != client.user: # Check to make sure the bot can't trigger its own commands
        ...
        #Command to literally just join a voice channel
        elif message.content.startswith(prefix+'join'):
            await join_voice(message.author.voice.voice_channel)
